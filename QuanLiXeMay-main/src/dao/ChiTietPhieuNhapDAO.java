...
public class ChiTietPhieuNhapDAO implements DAOInterface<ChiTietPhieu> {
    ...
    public int insert(ChiTietPhieu t) {
        ...
        try {
            ...
            String sql = "INSERT INTO ChiTietPhieuNhap (maPhieu, MaXe, soLuong, donGia) VALUES (?,?,?,?)";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getMaPhieu());
            pst.setString(2, t.getMaXe());
    ...
    public int update(ChiTietPhieu t) {
        ...
        try {
            Connection con = JDBCUtil.getConnection();
            String sql = "UPDATE ChiTietPhieuNhap SET maPhieu=?, MaXe=?, soLuong=?, donGia = ?  WHERE maPhieu=? AND MaXe=?";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getMaPhieu());
            pst.setString(2, t.getMaXe());
            pst.setInt(3, t.getSoLuong());
    ...
    public ArrayList<ChiTietPhieu> selectAll(String t) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ChiTietPhieu ctp = new ChiTietPhieu(maPhieu, MaXe, soLuong, donGia);
                ketQua.add(ctp);
            }
            JDBCUtil.closeConnection(con);
        } catch (Exception e) {
    ...
    public ChiTietPhieu selectById(String t) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int soLuong = rs.getInt("soLuong");
                double donGia = rs.getDouble("donGia");
                ketQua = new ChiTietPhieu(maPhieu, MaXe, soLuong, donGia);
            }
        } catch (Exception e) {
            // TODO: handle exception
            e.printStackTrace();
